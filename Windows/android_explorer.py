...
class ADB:
    ...
    def run(self, args, timeout=60, check=True):
        ...
        try:
            ...
            raise ADBError("adb.exe was not found. Install Android Platform Tools and add adb to PATH.")
        finally:
            if proc is not None:
                with self._process_lock:
...
class Explorer(QMainWindow):
    def __init__(self):
        ...
        # Windows Explorer-style frameless shell:
        # the tab strip itself acts as the draggable title bar.
    ...
    def show_settings_menu(self, global_pos=None):
        ...
        adb_action = menu.addAction("Select adb.exe...")
        scrcpy_action = menu.addAction("Select scrcpy.exe...")
        ...
        if action == theme_action:
            ...
            filename, _ = QFileDialog.getOpenFileName(
                self, "Select adb.exe", "", "ADB executable (adb.exe);;All files (*.*)"
            )
            if filename:
